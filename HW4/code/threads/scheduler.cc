...
Scheduler::FindNextToRun()
{
    ...
    if (readyList->IsEmpty()) {
        ...
        DEBUG(dbgSche, "[B] Tick [" << kernel->stats->totalTicks << "]: Process [" << readyList->Front()->getName() << "] is removed from queue"); // HW4+
...
Scheduler::Run(Thread* nextThread, bool finishing)
{
    ...
    DEBUG(dbgSche, "[C] Tick [" << kernel->stats->totalTicks << "]: Process [" << nextThread->getName() << "] is now selected for execution, Process [" << oldThread->getName() << "] is replaced."); // HW4+
