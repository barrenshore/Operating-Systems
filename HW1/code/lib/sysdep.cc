...
{
    int fd = open(name, O_RDWR|O_CREAT|O_TRUNC, 0666); // success: 0, fail: -1

    ASSERT(fd >= 0); 
    return fd; // success: 0, fail: -1
}
...
{
    int fd = open(name, O_RDWR, 0); // success: 3 or higher, fail: -1

    ASSERT(!crashOnError || fd >= 0);
    return fd; // success: 3 or higher, fail: -1
}
...
{
    int retVal = read(fd, buffer, nBytes); // success: 讀入數, fail: -1
...
{
    return read(fd, buffer, nBytes); // success: 讀入數, fail: -1
}
...
{
    int retVal = write(fd, buffer, nBytes); // success: 寫入數, fail: -1
    ASSERT(retVal == nBytes);
}

//----------------------------------------------------------------------
// WritePartial
// 	Write characters to an open file, returning as many as are
//	available.
//----------------------------------------------------------------------
...
{
    int retVal = lseek(fd, offset, whence); // success: 當前位置, fail: -1
    ASSERT(retVal >= 0);
}
...
Close(int fd)
{
    int retVal = close(fd); // success: 0, fail: -1
    ASSERT(retVal >= 0); 
    return retVal;
}
...
//----------------------------------------------------------------------
// ClosePartial
// 	Close a file, returning as many as are
//	available.
//----------------------------------------------------------------------
